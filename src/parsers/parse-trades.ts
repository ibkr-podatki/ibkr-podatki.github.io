import type { AssetType, ParsedTrade } from './types';

export const parseTradesTable = (doc: Document): Array<ParsedTrade> => {
	// Find the element with id starting with tblTransactions_
	const tradesContainer = Array.from(doc.querySelectorAll('[id^="tblTransactions_"]')).find(el =>
		el.id.startsWith('tblTransactions_')
	);
	...
	const trades: Array<ParsedTrade> = [];
	let currentAssetType: string | undefined = undefined;
		...
		if (cells.length >= 11) {
			...
			if (symbol && date && quantity !== null && tradePrice !== null && proceeds !== null) {
				trades.push({
					...
					realizedPL: realizedPL ?? undefined,
					mtmPL: mtmPL ?? undefined,
					code: code ?? undefined,
					assetType: currentAssetType as AssetType,

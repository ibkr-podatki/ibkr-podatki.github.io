import type { ParsedCorporateAction } from './types';

export const parseCorporateActionsTable = (doc: Document): Array<ParsedCorporateAction> => {
	// Find the element with id starting with tblCorporateActions_
	const container = Array.from(doc.querySelectorAll('[id^="tblCorporateActions_"]')).find(el =>
		el.id.startsWith('tblCorporateActions_')
	);
	...
	const actions: Array<ParsedCorporateAction> = [];

...
import type { ParsedWithholdingTax } from './types';

export const parseWithholdingTaxTable = (doc: Document): Array<ParsedWithholdingTax> => {
	// Find the element with id starting with tblWithholdingTax_
	const withholdingTaxContainer = Array.from(
		doc.querySelectorAll('[id^="tblWithholdingTax_"]')
	).find(el => el.id.startsWith('tblWithholdingTax_'));
	...
	const withholdingTaxes: Array<ParsedWithholdingTax> = [];

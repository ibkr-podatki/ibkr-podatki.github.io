import type { ParsedDividend } from './types';
import { parseTicker } from '../utils/utils';

export const parseDividendsTable = (doc: Document): Array<ParsedDividend> => {
	// Find the element with id starting with tblCombDiv
	const dividendsContainer = Array.from(doc.querySelectorAll('[id^="tblCombDiv"]')).find(el =>
		el.id.startsWith('tblCombDiv')
	);
	...
	const dividends: Array<ParsedDividend> = [];
